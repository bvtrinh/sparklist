...
router.post('/create', (req, res) => {
    ...
    if(errors.length > 0) {
        ...
        Group.find({admin: req.user.email, name: groupName})
            .then(group => {
                if (group.length != 0) {
                    // user already has group with entered name -> error
                    errors.push({msg: "You already created a group with this name. Please try again with a different name."});
                    res.render('pages/createGroup', {
                        errors,
                        invites,
                        visibility
...
router.get('/', authCheck, (req, res) => {
    ...
    Group.find({$or:[ {admin:req.user.email}, {members: req.user.email}]})
        .then(groups => {
            if (groups.length == 0) {
...
router.get('/manage/', (req, res) => {
    ...
    Group.findById(groupID)
        .then(group => {
            if(group){
                res.render('pages/updateGroup', { group });
            }
        });
});
